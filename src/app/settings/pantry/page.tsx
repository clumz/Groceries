    ...
    <div className="min-h-screen bg-surface-secondary pb-24">
      <div className="bg-surface px-5 pt-14 pb-4 sticky top-0 z-10 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-ink">Kitchen & Pantry</h1>
        </div>
          ...
          {(["staples", "pantry"] as const).map((tab) => (
            <button
              ...
              className={clsx(
                ...
                activeTab === tab ? "bg-surface text-ink shadow-sm" : "text-ink-secondary"
              )}
            >
        ...
        {activeTab === "staples" && (
          <>
            ...
            {STAPLE_GROUPS.map((group) => (
              <div key={group.label} className="bg-surface rounded-3xl overflow-hidden shadow-card">
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-slate-50 dark:hover:bg-surface-tertiary transition-colors text-left"
                      >
                        <span className={clsx("text-sm capitalize", active ? "text-ink font-medium" : "text-ink-secondary")}>{item}</span>
                        <div className={clsx("w-11 h-6 rounded-full transition-colors relative flex-shrink-0", active ? "bg-brand-600" : "bg-slate-200")}>
                          <div className={clsx("absolute top-0.5 w-5 h-5 rounded-full bg-surface shadow transition-all duration-150", active ? "left-[calc(100%-1.375rem)]" : "left-0.5")} />
                        </div>
                      </button>
                    );
                  })}
            ...
            <div className="bg-surface rounded-3xl shadow-card px-4 py-4">
        ...
        {activeTab === "pantry" && (
          <>
            ...
            {pantryItems.length === 0 ? (
              <div className="bg-surface rounded-3xl shadow-card px-5 py-12 flex flex-col items-center gap-3 text-center">
